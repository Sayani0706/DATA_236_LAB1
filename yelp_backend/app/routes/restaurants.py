...
def search_restaurants(
    ...
    if sort_by == "rating":
        ...
        result.sort(key=lambda x: len(x["restaurant"].pricing_tier) if x["restaurant"].pricing_tier else 0)
...
def upload_restaurant_photo(restaurant_id: int, file: UploadFile = File(...), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    r = db.query(Restaurant).filter(Restaurant.id == restaurant_id).first()
    if not r:
        raise HTTPException(status_code=404, detail="Not found")
