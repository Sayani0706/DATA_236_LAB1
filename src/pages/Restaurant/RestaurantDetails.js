...
import { addFavourite, removeFavourite, selectFavouriteIds } from '../../store/slices/favouritesSlice.js';
...
const RestaurantDetails = () => {
  ...
  const isFav                 = favouriteIds.includes(id);
  ...
  const handleFavorite = () => {
    if (!token) { setFavMsg('Please log in to save favourites.'); return; }
    if (isFav) {
  ...
  if (loading) return (
    <div className="text-center mt-5"><div className="spinner-border text-danger" /></div>
  );
  if (!data) return (
  ...
  return (
    <div className="container mt-4">
      ...
      {restaurant.photos && restaurant.photos.length > 0 && (
        <div className="mb-4 d-flex flex-wrap gap-3">
          {restaurant.photos.map((photo, idx) => (
            <img key={idx} src={`${API_URL}${photo.photo_url}`} alt={`${restaurant.name} photo`}
      ...
      {favMsg && (
        <div className={`alert py-2 ${favMsg.includes('not') || favMsg.includes('log') ? 'alert-warning' : 'alert-success'}`}>
          {favMsg}
        </div>
      )}
      ...
      <div className="row">
        ...
        <div className="col-md-5">
          {token ? (
            <WriteReview restaurantId={id} onReviewAdded={fetchDetails} />
          ) : (
